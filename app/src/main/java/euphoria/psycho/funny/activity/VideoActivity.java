...
import static euphoria.psycho.funny.FileItemComparator.SORT_BY_MODIFIED_TIME;
import static euphoria.psycho.funny.FileItemComparator.SORT_BY_NAME;
import static euphoria.psycho.funny.FileItemComparator.SORT_BY_SIZE;
import static euphoria.psycho.funny.util.Simple.isNullOrEmpty;
...
public class VideoActivity extends BaseVideoActivity implements
    ...
    private SingleSampleMediaSource buildSubtitleMediaSource(File file) {
        File subtitleFile = new File(FileUtils.changeExtension(file.getAbsolutePath(), "srt"));
            ...
            public int compare(File o1, File o2) {
                ...
                if (mSortBy == SORT_BY_MODIFIED_TIME) {
                    if (mSortDirection == SORT_BY_DESCENDING) {
                        return o1.lastModified() <= o2.lastModified() ? 1 : -1;//mCollator.compare(o1.getName(), o2.getName());
...
//                && mPlayer.getCurrentPosition() <= MAX_POSITION_FOR_SEEK_TO_PREVIOUS
    ...
    private void savePosition() {
        ...
        mBookmarker.setBookmark(uri, mPlayer.getCurrentPosition());
    }
    ...
    private void seekToLastedState() {
        ...
        if (getSupportActionBar() != null) {
            getSupportActionBar().setTitle(FileUtils.getFileName(uri));
        }
        updateNavigation();
    }
    ...
    public static void openDeleteVideoDialog(Context context, String
            fileName, FileUtils.OperationResultCallback callback) {
